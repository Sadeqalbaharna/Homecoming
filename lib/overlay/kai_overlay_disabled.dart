// kai_overlay_disabled.dart
// DISABLED — Windows system overlay (floats above all apps like Shimeji).
// SYSTEM_ALERT_WINDOW permission is already in AndroidManifest, so the Android
// equivalent (Kai floating over other apps) is a viable future feature.
// Re-enable by wiring this as an entry point: flutter run -t lib/overlay/kai_overlay_disabled.dart
...
import 'services/media/audio_player_service.dart';
...
class _OverlayWidgetState extends State<OverlayWidget> with TickerProviderStateMixin {
  ...
  Future<void> _initializeVoiceActivation() async {
    try {
      ...
      _voiceActivation.onWakeWordDetected.listen((completeMessage) async {
        print('🎯 [WAKE WORD] Detected complete message: "$completeMessage"');
        
        // Send the complete transcription to Kai (including "hey kai")
        // This ensures Kai receives the wake word as an actual message
        if (completeMessage.isNotEmpty) {
          await _sendMessage(completeMessage);
          // TTS will play automatically from _sendMessage
        } else {
          // Fallback: if somehow empty, send default greeting
          await _sendMessage("hey kai");
        }
      });
  ...
  void initState() {
    super.initState();
    KaiStateService().setSurface('desktop');
    EmotionalEventService().setSurface('desktop');
                  ...
                  Positioned(
                    ...
                    child: SafeArea(
                      child: FloatingActionButton(
                        ...
                        child: const Icon(Icons.close, color: Colors.w