...
class KaiCoreSidecarManager {
  KaiCoreSidecarManager({
    ...
    int readinessAttempts = 25,
    Iterable<String> Function()? executableCandidates,
  })  : client = client ?? KaiCoreClient(),
        _processStarter = processStarter ?? Process.start,
        _delay = delay ?? Future<void>.delayed,
        _readinessAttempts = readinessAttempts,
        _executableCandidatesOverride = executableCandidates;
  ...
  final int _readinessAttempts;
  final Iterable<String> Function()? _executableCandidatesOverride;
  ...
  Future<bool> ensureAvailable() async {
    ...
    for (final candidate
        in (_executableCandidatesOverride?.call() ?? _executableCandidates())) {
      ...
      for (var attempt = 0; attempt < _readinessAttempts; attempt++) {
