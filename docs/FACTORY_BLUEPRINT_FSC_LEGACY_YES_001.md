...
- Authorization: `BPA-20260809-FSC-LEGACY-YES-001`
- Sponsor: Sadeq
- Captured: `2026-08-09T14:20:02.372+03:00`
- Evidence: supervising Factory record states that Sadeq originated the concept,
  previously voted YES, and explicitly requested moving this exact concept to
  Blueprint.

These identifiers are a new legacy-recovery binding. They are not represented
as the missing historical run or concept ID. The registered authorization is
accepted only for this exact Factory run, scan session, and candidate. The
prior YES is stored as a reconstructed verdict, not a claim that the sponsor
cast it inside this new session, and it is excluded from future trait-ranking
weights.
