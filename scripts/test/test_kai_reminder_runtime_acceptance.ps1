...
try {
    ...
    Write-Json $capableButOld @{ ok = $true; startedAt = '2026-08-07T12:00:00Z'; capabilities = @('presence', 'scheduled_commitments') }
    ...
    $boundary = @('-ExpectedCoreStartedAfterUtc','2026-08-08T11:00:00Z')
    Invoke-Case 'rebuilt-preflight' (@('-Mode','Preflight','-HealthPath',$health,'-CommitmentsPath',$created) + $boundary) 0 '[PASS]' | Out-Null
    Invoke-Case 'missing-runtime-boundary' @('-Mode','Preflight','-HealthPath',$health,'-CommitmentsPath',$created) 1 'required for every acceptance mode' | Out-Null
    Invoke-Case 'stale-runtime' @('-Mode','Preflight','-HealthPath',$stale,'-CommitmentsPath',$created) 2 'stale' | Out-Null
    Invoke-Case 'capable-but-old-runtime' @('-Mode','Preflight','-HealthPath',$capableButOld,'-CommitmentsPath',$created,'-ExpectedCoreStartedAfterUtc','2026-08-08T11:00:00Z') 2 'not newer' | Out-Null

    $createdArgs = @('-Mode','Created','-HealthPath',$health,'-CommitmentsPath',$created,'-CommitmentId','brief018-test','-ExpectedTextSha256',$textHash,'-Marker','TEXTMARKER-UNIQUE') + $boundary
    ...
    $survivedArgs = @('-Mode','Survived','-HealthPath',$health,'-CommitmentsPath',$created,'-CommitmentId','brief018-test','-ExpectedTextSha256',$textHash,'-ExpectedPromiseFingerprint',$fingerprint,'-Marker','TEXTMARKER-UNIQUE') + $boundary
    Invoke-Case 'survived' $survivedArgs 0 'survived' | Out-Null
    Invoke-Case 'missing-survival-fingerprint' (@('-Mode','Survived','-HealthPath',$health,'-CommitmentsPath',$created,'-CommitmentId','brief018-test','-ExpectedTextSha256',$textHash,'-Marker','TEXTMARKER-UNIQUE') + $boundary) 1 'required after the restart' | Out-Null
    Invoke-Case 'fingerprint-drift' (@('-Mode','Survived','-HealthPath',$health,'-CommitmentsPath',$created,'-CommitmentId','brief018-test','-ExpectedTextSha256',$textHash,'-ExpectedPromiseFingerprint',('0' * 64),'-Marker','TEXTMARKER-UNIQUE') + $boundary) 1 'changed across the restart' | Out-Null
    Invoke-Case 'text-drift' (@('-Mode','Created','-HealthPath',$health,'-CommitmentsPath',$created,'-CommitmentId','brief018-test','-ExpectedTextSha256',('0' * 64),'-Marker','TEXTMARKER-UNIQUE') + $boundary) 1 'byte-for-byte hash' | Out-Null
    ...
    Invoke-Case 'duplicate-run-commitment' (@('-Mode','Created','-HealthPath',$health,'-CommitmentsPath',$duplicates,'-CommitmentId','brief018-test','-ExpectedTextSha256',$textHash,'-Marker','TEXTMARKER-UNIQUE') + $boundary) 1 'does not map to exactly one' | Out-Null
    ...
    @($dispatch, $uncorrelatedLeak) | ForEach-Object { $_ | ConvertTo-Json -Compress -Depth 8 } | Set-Content -LiteralPath (Join-Path $journal 'kai-operations.jsonl') -Encoding UTF8
